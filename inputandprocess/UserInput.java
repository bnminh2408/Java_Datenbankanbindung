...
public class UserInput {
	...
	public Mitarbeiter setMitarbeiter(Scanner sc) {
		String id = inputValues("Bitte ID eingeben: ", sc);
		while (!validID(id)) {
			id = inputValues("Bitte ID eingeben: ", sc);
		}
		...
		while (!validPLZ(plz)) {
			plz = inputValues("Bitte Plz eingeben: ", sc);
		}
	...
	public int secondProcess() {
		...
		try {
			input = Integer.parseInt(userInput.inputValues("Was moechten sie tun ?\n" + "(1) Insert\n" + "(2) Delete\n"
					+ "(3) Select All\n" + "(4) Select by ID\n" + "(5) Quit\n" + "Bitte nur Zahlen 1 bis 5 eingeben: ",
					sc));
			return input;
		} catch (Exception e) {
			System.out.println("\nKeine invalide Eingabe!");
		}
		return input;
	}
	...
	public boolean validPLZ(String plz) {
		try {
			...
			System.out.println("Keine gueltige PLZ! \n");
			return false;
		}
	}
	...
	public boolean validID(String id) {
		try {
			...
			System.out.println("Keine guetige ID! \n");
			return false;
		}
	}
